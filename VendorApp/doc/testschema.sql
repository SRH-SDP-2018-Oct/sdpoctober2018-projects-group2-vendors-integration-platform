...
CREATE TABLE user_type (
   id TINYINT UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT COMMENT 'User type id. It will be used as referenece in the users table',
...
CREATE TABLE user_master (
  ...
  uPwd TEXT NOT NULL DEFAULT '' COMMENT 'User Password',
  ...
  FOREIGN KEY (userType) REFERENCES user_type(id) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=INNODB DEFAULT CHARSET=latin1 ;
...
SELECT user_type.id, '1', '1', 'root', 'root', 'root', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
FROM user_type
WHERE user_type.typeName = 'admin';

-- Entry -> SYSTEM User
INSERT INTO user_master (userType, createdBy, modifiedBy, username, firstName, lastName, createdOn, modifiedOn)
SELECT user_type.id , um.userId, um.userId, 'system', 'system', 'system', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
